...
from water import cal_d5_all, compute_sg_all, compute_sk_all, compute_lsi_all
...
if __name__ == '__main__':
    ...
    for structure in structures:
        ...
        showImg = False
        d5s = cal_d5_all(samples, r_max=4.5)
        np.savez('{}/d5.npz'.format(outputFolder), d5=d5s)
        if showImg:
            # Plot histogram
            bins = 50
            label = 'd5_{}'.format(structure)
            legend = 'd5 ({})'.format(structure) if structure  !=  'P' else 'd5 (Reference)'
            plt.hist(d5s, bins=bins, color=color)
            plt.xlabel('d5')
            plt.title('d5 distribution')
            plt.legend([legend])
            plt.show()

        sgs = compute_sg_all(samples, r_max=3.7)
        np.savez('{}/sg.npz'.format(outputFolder), sg=sgs)
        if showImg:
            # Plot histogram
            bins = 50
            label = 'sg_{}'.format(structure)
            legend = 'q ({})'.format(structure) if structure  !=  'P' else 'sg (Reference)'
            plt.hist(sgs, bins=bins, color=color)
            plt.xlabel('q')
            plt.title('q distribution')
            plt.legend([legend])
            plt.show()

        sks = compute_sk_all(samples, r_max=3.7)
        np.savez('{}/sk.npz'.format(outputFolder), sk=sks)
        if showImg:
            # Plot histogram
            bins = 50
            label = 'sk_{}'.format(structure)
            legend = 'sk ({})'.format(structure) if structure  !=  'P' else 'sk (Reference)'
            plt.hist(sks, bins=bins, color=color)
            plt.xlabel('sk')
            plt.title('sk distribution')
            plt.legend([legend])
            plt.show()

        lsis = compute_lsi_all(samples, r_max=4.5)
        np.savez('{}/lsi.npz'.format(outputFolder), lsi=lsis)
        if showImg:
            # Plot histogram
            bins = 50
            label = 'lsi_{}'.format(structure)
            legend = 'lsi ({})'.format(structure) if structure  !=  'P' else 'lsi (Reference)'
            plt.hist(lsis, bins=bins, color=color)
            plt.xlabel('lsi')
